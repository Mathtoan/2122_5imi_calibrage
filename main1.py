...
input_filename_moving = os.path.join(image_dir, 'BrainProtonDensitySliceShifted13x17y.png')
...
registration_filter.SetTransform(itk.TranslationTransform[itk.D, 2].New()) # Transformation
...
itk.imwrite(output_image, os.path.join(image_output_dir, 'image_recalee.png'))

itk.imwrite(itk.AbsoluteValueDifferenceImageFilter(output_image, moving_image), os.path.join(image_output_dir,'moving_diff.png'))
itk.imwrite(itk.AbsoluteValueDifferenceImageFilter(output_image, fixed_image), os.path.join(image_output_dir,'fixed_diff.png'))